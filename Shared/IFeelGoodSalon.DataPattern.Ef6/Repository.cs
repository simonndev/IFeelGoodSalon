...
namespace IFeelGoodSalon.DataPattern.Ef6
{
    public class Repository<TEntity> : IRepositoryAsync<TEntity> where TEntity : class, IObservableEntity
    {
        ...
        private readonly IObservableDbContextAsync _dbContext;
        ...
        {
            entity.ModelState = EntityModelState.Added; ;
        ...
        {
            entity.ModelState = EntityModelState.Modified;
            _dbSet.Attach(entity);
            _dbContext.SyncObjectState(entity);
        }
        ...
        {
            entity.ModelState = EntityModelState.Deleted;
            _dbSet.Attach(entity);
            _dbContext.SyncObjectState(entity);
        }
        ...
        public virtual IQueryFluent<TEntity> Query(IQueryLogic<TEntity> queryObject)
        {
        ...
        private void SyncObjectGraph(object entity) // scan object graph for all 
        {
            ...
            var objectState = entity as IObservableEntity;

            // discovered entity with ObjectState.Added, sync this with provider e.g. EF
            if (objectState != null && objectState.ModelState == EntityModelState.Added)
            {
                _dbContext.SyncObjectState((IObservableEntity)entity);
            }
            ...
            foreach (var prop in entity.GetType().GetProperties())
            {
                ...
                var trackableRef = prop.GetValue(entity, null) as IObservableEntity;
                if (trackableRef != null)
                {
                    // discovered entity with ObjectState.Added, sync this with provider e.g. EF
                    if (trackableRef.ModelState == EntityModelState.Added)
                        _dbContext.SyncObjectState((IObservableEntity)entity);
                ...
                var items = prop.GetValue(entity, null) as IEnumerable<IObservableEntity>;
