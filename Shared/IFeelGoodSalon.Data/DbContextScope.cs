...
{
    public class DbContextScope : IDbContextScopeAsync
    {
        ...
        private DbContextManager _manager;
        ...
        public DbContextScope(DbContextScopeOption joiningOption, bool readOnly, IsolationLevel? isolationLevel, IDbContextFactory dbContextFactory = null)
        {
            ...
            {
                if (_parentScope._isReadOnly && !this._isReadOnly)
                {
                    throw new InvalidOperationException("Cannot nest a read/write DbContextScope within a read-only DbContextScope.");
                }

                _isNested = true;
                _manager = _parentScope._manager;
            }
            else
            {
                _isNested = false;
                _manager = new DbContextManager(readOnly, isolationLevel, dbContextFactory);
            }
        ...
        public async Task<int> SaveChangesAsync(CancellationToken cancelToken)
        {
            ...
            if (!_isNested)
            {
                effort = await CommitInternalAsync(cancelToken).ConfigureAwait(false);
            }
        ...
        public void ReloadParentScope(IEnumerable entities)
        {
            if (entities == null)
            {
                return;
            ...
            if (_isNested || _parentScope == null)
            {
            ...
            foreach (IObjectContextAdapter contextInCurrentScope in _manager.InitializedDbContexts.Values)
            {
                var correspondingParentContext =
                    _parentScope._manager.InitializedDbContexts.Values.SingleOrDefault(parentContext => parentContext.GetType() == contextInCurrentScope.GetType())
                    as IObjectContextAdapter;

                if (correspondingParentContext == null)
                {
                    // No DbContext of this type has been created in the parent scope yet.
                    // So no need to refresh anything for this DbContext type.
                    continue;
                }
                ...
                foreach (var entity in entities)
                {
                    ...
                    bool succeed = contextInCurrentScope.ObjectContext.ObjectStateManager.TryGetObjectStateEntry(entity, out stateInCurrentScope);
                    if (succeed)
                    {
                        ...
                        succeed = correspondingParentContext.ObjectContext.ObjectStateManager.TryGetObjectStateEntry(entity, out stateInParentScope);
                        if (succeed)
                        {
        ...
        public async Task ReloadParentScopeAsync(IEnumerable entities)
        {
            ...
            foreach (IObjectContextAdapter contextInCurrentScope in _manager.InitializedDbContexts.Values)
            {
                ...
                {
                    ObjectStateEntry stateInCurrentScope;
                    bool succeed = contextInCurrentScope.ObjectContext.ObjectStateManager.TryGetObjectStateEntry(toRefresh, out stateInCurrentScope);
                    if (succeed)
                    {
                        var key = stateInCurrentScope.EntityKey;

                        ObjectStateEntry stateInParentScope;
                        succeed = correspondingParentContext.ObjectContext.ObjectStateManager.TryGetObjectStateEntry(key, out stateInParentScope);
                        if (succeed)
                        {
                            if (stateInParentScope.State == EntityState.Unchanged)
                            {
...
" + Environment.StackTrace;
        ...
        private static readonly ConditionalWeakTable<DbContextInstanceIdentifier, DbContextScope> DbContextScopeInstances =
            new ConditionalWeakTable<DbContextInstanceIdentifier, DbContextScope>();

        private DbContextInstanceIdentifier _instanceIdentifier = new DbContextInstanceIdentifier();
        ...
        internal static void SetAmbientScope(DbContextScope newAmbientScope)
        {
            ...
            var current = CallContext.LogicalGetData(AmbientDbContextScopeKey) as DbContextInstanceIdentifier;
        ...
        internal static DbContextScope GetAmbientScope()
        {
            ...
            var instanceIdentifier = CallContext.LogicalGetData(AmbientDbContextScopeKey) as DbContextInstanceIdentifier;
            if (instanceIdentifier == null)
            {
                // either no ambient context has been set or
                // we've crossed an app domain boundary and have (intentionally) lost the ambient context
                return null;
            }

            // retrieves the DbContextScope instance corresponding to this identifier
